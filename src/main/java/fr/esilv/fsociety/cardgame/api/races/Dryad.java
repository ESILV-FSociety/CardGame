package fr.esilv.fsociety.cardgame.api.races;

import fr.esilv.fsociety.cardgame.api.Card;
import fr.esilv.fsociety.cardgame.api.Game;

public class Dryad extends Card {


	public Dryad() {
		super("Dryad");
	}

	public void activatePower(Game game){
        // stole a card in front of your opponent and add it in front of you without activating its power.
        if (game.getOpponnent().getBoard().getKingdom().size() != 0){
            Card cardToAdd =
                    game.getOpponnent().getBoard().getKingdom().remove(0);
            game.getCurrentPlayer().getBoard().addCardToKingdom(cardToAdd);
        }
    }

}




